...
def display_stage_selector():
    ...
    selected_stage = st.selectbox(
        "Which stage is your plant currently in?",
    ...
    if save_button:
        ...
        st.session_state.student_group_plant_stage = selected_stage
...
def main():
    ...
    st.write(
        "Monitor your plant using your "
        "Flower Care 4-in-1 sensor."
    )

    if "student_group_plant_stage" not in st.session_state:
        st.session_state.student_group_plant_stage = load_saved_stage()

    stage = st.session_state.student_group_plant_stage
    ...
    if change_stage_button:
        st.session_state.student_group_plant_stage = None
        st.session_state.pop("plant_report", None)
        st.rerun()
